...
WORKFLOW = Path(__file__).resolve().parents[1] / '.github/workflows/sync-upstream.yml'
CONFIG = yaml.safe_load(WORKFLOW.read_text())
STEPS = [s for s in CONFIG['jobs']['sync']['steps'] if 'run' in s and s['name'] != 'Validate caller inputs']
VALIDATE = next(s['run'] for s in CONFIG['jobs']['sync']['steps'] if s['name'] == 'Validate caller inputs')
BASE_BRANCH = os.environ.get('SYNC_TEST_BASE_BRANCH', 'develop')
UPSTREAM_REPO = os.environ.get('SYNC_TEST_UPSTREAM_REPO', 'seerr-team/seerr')
...
''')
    ...
    def test_caller_input_validation(self):
        self.histories()
        cases = [({}, True), ({'UPSTREAM_REPO': 'evil/repo;touch bad'}, False),
                 ({'UPSTREAM_REPO': '../repo'}, False), ({'BASE_BRANCH': '--all'}, False),
                 ({'BASE_BRANCH': 'refs/../bad'}, False),
                 ({'REQUESTED_REF': 'v1.2.3-rc.1'}, False)]
        for overrides, passes in cases:
            with self.subTest(overrides=overrides):
                r = subprocess.run(['bash', '-c', VALIDATE], cwd=self.work,
                                   env=dict(self.env, **overrides), text=True, capture_output=True)
                self.assertEqual(r.returncode == 0, passes, r.stderr)
        self.assertEqual(self.gh_calls(), [])

    def test_reusable_workflow_only_and_input_bindings(self):
        self.assertEqual(set(CONFIG.get('on', CONFIG.get(True))), {'workflow_call'})
        self.assertEqual(CONFIG['env']['UPSTREAM_REPO'], '${{ inputs.upstream_repo }}')
        self.assertEqual(CONFIG['env']['BASE_BRANCH'], '${{ inputs.base_branch }}')
        checkout = next(s for s in CONFIG['jobs']['sync']['steps'] if s.get('uses', '').startswith('actions/checkout@'))
        self.assertEqual(checkout['with']['ref'], '${{ inputs.base_branch }}')
        self.assertNotIn('repository', checkout['with'])
        self.assertNotIn('secrets', CONFIG.get('on', CONFIG.get(True))['workflow_call'])
